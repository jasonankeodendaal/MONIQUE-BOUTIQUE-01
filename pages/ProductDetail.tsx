...
const ProductDetail: React.FC = () => {
  ...
  const handleAffiliateClick = (e: React.MouseEvent) => {
    ...
    // 1. Log conversion intent
    logEvent('click', `Product: ${product.name}`, 'Product Page', { interactionType: 'Affiliate Redirect' });
    
    // 2. Show Trust Overlay
    setIsRedirecting(true);
    
    // 3. Perform Redirect after 2 seconds
    setTimeout(() => {
  ...
  const sharePayload = useMemo(() => {
      ...
      return { title: product.name, text: [`✨ ${product.name} ✨`, '', safeDesc, '', `💎 Price: ${priceString} ${discountString ? `(${discountString} OFF 🔥)` : ''}`, '', '👇 View details & Acquire:'].join('\n'), url: window.location.href };
  }, [product]);
  ...
  const handleSubmitReview = async (e: React.FormEvent) => {
    ...
    if (isSupabaseConfigured) { await supabase.from('reviews').insert(review); } else {
        const allReviews: Review[] = JSON.parse(localStorage.getItem('site_reviews') || '[]');
  ...
  const handleShare = async () => {
    ...
    if (navigator.share) {
      try {
        const shareData: ShareData = { title: title, text: `${text}\n${url}` };
        const imgUrl = product.media?.[activeMediaIndex]?.url || product.media?.[0]?.url;
        if (imgUrl) {
          try {
            const response = await fetch(imgUrl);
            const blob = await response.blob();
            const filename = `${(product.name || 'product').replace(/[^a-z0-9]/gi, '_').toLowerCase()}.jpg`;
            const file = new File([blob], filename, { type: blob.type });
            if (navigator.canShare && navigator.canShare({ files: [file] })) shareData.files = [file];
          } catch (e) {}
        }
        await navigator.share(shareData);
      } catch (err) { if ((err as Error).name !== 'AbortError') setIsShareOpen(true); }
    } else { setIsShareOpen(true); }
  };

  const averageRating = useMemo(() => reviews.length === 0 ? 0 : Math.round(reviews.reduce((acc, r) => acc + r.rating, 0) / reviews.length), [reviews]);

  if (!isDataLoaded) return <div className="min-h-screen flex items-center justify-center bg-[#FDFCFB]"><div className="flex flex-col items-center gap-4"><Loader2 className="animate-spin text-slate-900" size={32} /><p className="text-[10px] font-black uppercase tracking-widest text-slate-400 animate-pulse">Loading Collection...</p></div></div>;
  if (!product) return <div className="min-h-screen flex items-center justify-center bg-white text-center p-6"><div><Package size={64} className="text-slate-200 mx-auto mb-6" /><h2 className="text-3xl font-serif mb-4">Piece Not Found</h2><button onClick={() => navigate('/products')} className="text-primary font-bold uppercase tracking-widest text-xs">Return to Collection</button></div></div>;
  ...
  return (
    <main className={`min-h-screen bg-[#FDFCFB] pt-28 md:pt-32 transition-opacity duration-1000 ${isLoaded ? 'opacity-100' : 'opacity-0'}`}>
      {isRedirecting && <TrustOverlay destination={product.affiliateLink.includes('amazon') ? 'Amazon' : 'Verified Partner'} />}
      ...
      <div className="flex flex-col lg:flex-row min-h-[calc(100vh-8rem)] lg:h-[calc(100vh-8rem)] lg:overflow-hidden">
        <div className="w-full lg:w-3/5 h-[45vh] md:h-[55vh] lg:h-full relative bg-slate-100 overflow-hidden group rounded-t-[2.5rem] lg:rounded-tr-none lg:rounded-l-[2.5rem]">
          ...
          {media.length > 1 && (
            <>
              <div className="absolute inset-y-0 left-0 w-24 md:w-32 flex items-center justify-center z-20 opacity-0 group-hover:opacity-100 transition-opacity"><button onClick={() => setActiveMediaIndex(prev => (prev === 0 ? media.length - 1 : prev - 1))} className="w-12 h-12 md:w-16 md:h-16 bg-white/10 backdrop-blur-xl border border-white/20 rounded-full flex items-center justify-center text-white hover:bg-white hover:text-slate-900 transition-all"><ChevronLeft size={24} /></button></div>
              <div className="absolute inset-y-0 right-0 w-24 md:w-32 flex items-center justify-center z-20 opacity-0 group-hover:opacity-100 transition-opacity"><button onClick={() => setActiveMediaIndex(prev => (prev === media.length - 1 ? 0 : prev + 1))} className="w-12 h-12 md:w-16 md:h-16 bg-white/10 backdrop-blur-xl border border-white/20 rounded-full flex items-center justify-center text-white hover:bg-white hover:text-slate-900 transition-all"><ChevronRight size={24} /></button></div>
              <div className="absolute bottom-6 md:bottom-10 left-1/2 -translate-x-1/2 flex gap-3 z-20">{media.map((_: any, i: number) => <button key={i} onClick={() => setActiveMediaIndex(i)} className={`h-1.5 transition-all duration-500 rounded-full ${i === activeMediaIndex ? 'w-8 bg-white' : 'w-2 bg-white/50 hover:bg-white/80'}`} />)}</div>
            </>
          )}
        </div>
          ...
          <div className="max-w-xl mx-auto space-y-8">
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                ...
                <button onClick={handleShare} className="p-2 rounded-full hover:bg-slate-50 text-slate-400 hover:text-slate-900 transition-colors"><Share2 size={18} /></button>
              </div>
              <h1 className="text-3xl md:text-5xl font-serif text-slate-900 leading-tight">{product.name}</h1>
              <div className="flex items-center gap-4"><span className="text-2xl md:text-3xl font-black text-slate-900">R {(product.price || 0).toLocaleString()}</span>{product.discountRules && product.discountRules.length > 0 && <span className="px-3 py-1 bg-red-100 text-red-600 rounded-full text-[10px] font-black uppercase tracking-widest">{product.discountRules[0].type === 'percentage' ? `-${product.discountRules[0].value}%` : `-R${product.discountRules[0].value}`}</span>}</div>
            </div>

            <div className="py-8 border-y border-slate-100 space-y-6">
               {product.isDirectSale ? (
                  <div className="space-y-6">
                      <div className="flex items-center justify-between"><span className="text-xs font-bold uppercase tracking-widest text-slate-500">Quantity</span><div className={`flex items-center bg-slate-100 rounded-xl ${isOutOfStock ? 'opacity-50 pointer-events-none' : ''}`}><button onClick={() => setQuantity(Math.max(1, quantity - 1))} className="w-10 h-10 flex items-center justify-center text-slate-500 hover:text-slate-900 transition-colors"><Minus size={14}/></button><span className="w-10 text-center font-bold text-slate-900">{quantity}</span><button onClick={() => setQuantity(Math.min((product.stockQuantity || 1), quantity + 1))} className="w-10 h-10 flex items-center justify-center text-slate-500 hover:text-slate-900 transition-colors"><Plus size={14}/></button></div></div>
            ...
            {product.specifications && Object.keys(product.specifications).length > 0 && (
               <div className="border border-slate-100 rounded-2xl overflow-hidden"><button onClick={() => setOpenAccordion(openAccordion === 'specs' ? null : 'specs')} className="w-full px-6 py-4 flex items-center justify-between bg-slate-50 hover:bg-slate-100 transition-colors"><span className="text-xs font-black uppercase tracking-widest text-slate-900">Technical Specifications</span>{openAccordion === 'specs' ? <Minus size={14}/> : <Plus size={14}/>}</button>{openAccordion === 'specs' && <div className="px-6 py-4 bg-white grid grid-cols-2 gap-4">{Object.entries(product.specifications).map(([key, value]) => <div key={key}><span className="block text-[9px] font-black uppercase text-slate-400 tracking-widest mb-1">{key}</span><span className="text-sm text-slate-700">{value}</span></div>)}</div>}</div>
            )}
            ...
            <div className="pt-8 space-y-8">
               ...
               {openAccordion === 'review' && (
                 <form onSubmit={handleSubmitReview} className="bg-slate-50 p-6 rounded-2xl border border-slate-100 space-y-4 animate-in slide-in-from-top-4">
                    <div className="space-y-2"><label className="text-[10px] font-black uppercase text-slate-400 tracking-widest">Rating</label><div className="flex gap-1">{[1,2,3,4,5].map(star => <button type="button" key={star} onClick={() => setNewReview({...newReview, rating: star})} className="focus:outline-none"><Star size={20} fill={star <= newReview.rating ? "#D4AF37" : "none"} className={star <= newReview.rating ? "text-primary" : "text-slate-300"} /></button>)}</div></div>
                    <div className="space-y-2"><label className="text-[10px] font-black uppercase text-slate-400 tracking-widest">Name</label><input type="text" required value={newReview.userName} onChange={e => setNewReview({...newReview, userName: e.target.value})} className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:border-slate-900 outline-none text-sm bg-white" placeholder="Your Name" /></div>
                    <div className="space-y-2"><label className="text-[10px] font-black uppercase text-slate-400 tracking-widest">Experience</label><textarea rows={3} required value={newReview.comment} onChange={e => setNewReview({...newReview, comment: e.target.value})} className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:border-slate-900 outline-none text-sm bg-white resize-none" placeholder="Share your thoughts..." /></div>
