...
const AccessDenied: React.FC = () => (
  <div className="flex flex-col items-center justify-center h-[50vh] text-center p-8 bg-slate-900 border border-slate-800 rounded-[2rem]">
    ...
    <p className="text-slate-500 max-w-sm text-sm">
      Your administrative profile does not have the required clearance to access this module. Please contact the system owner.
    </p>
  </div>
);
...
const Admin: React.FC = () => {
  ...
  useEffect(() => {
    if (showProductForm && productData.isDirectSale) {
        if (!hasPermission('catalog.direct_sales')) {
            setProductData(prev => ({ ...prev, isDirectSale: false }));
        }
    }
  }, [showProductForm, productData.isDirectSale, isOwner, myAdminProfile]);
  ...
  useEffect(() => {
    const fetchTraffic = async () => {
       ...
       } catch (e) { setTrafficEvents([]); }
    };
    if (hasPermission('analytics.view')) { fetchTraffic(); }
  ...
  const handleBulkStatusUpdate = async (newStatus: string) => { if(!hasPermission('sales.fulfillment')) return; for(const oid of selectedOrderIds) { const order = orders.find(o => o.id === oid); if(order) await updateData('orders', { ...order, status: newStatus }); } setSelectedOrderIds([]); };
  const handleSaveTracking = async () => { if(!viewingOrder || !hasPermission('sales.fulfillment')) return; const updated = { ...viewingOrder, courierName: trackingInfo.courier, trackingNumber: trackingInfo.tracking, status: 'shipped' as const }; await updateData('orders', updated); setViewingOrder(updated); alert(`Status updated to Shipped. Notification sent to ${viewingOrder.customerEmail}`); };
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 w-full max-w-7xl mx-auto text-left">
      <div className="flex flex-col md:flex-row justify-between items-end gap-6 mb-8">
         ...
         <div className="flex gap-3 w-full md:w-auto">
            {hasPermission('sales.export') && <button onClick={exportEnquiries} className="flex-1 md:flex-none justify-center px-6 py-3 bg-primary text-slate-900 rounded-xl font-bold text-xs uppercase tracking-widest hover:bg-white transition-colors flex items-center gap-2"><FileSpreadsheet size={16}/> Export CSV</button>}
         </div>
      </div>
        ...
        filteredEnquiries.map(e => (
          <div key={e.id} className={`bg-slate-900 border transition-all rounded-[2rem] md:rounded-[2.5rem] p-5 md:p-6 flex flex-col md:flex-row gap-6 text-left ${e.status === 'unread' ? 'border-primary/30 shadow-[0_10px_30px_-10px_rgba(var(--primary-rgb),0.1)]' : 'border-slate-800'}`}>
            ...
            <div className="flex gap-2 items-start w-full md:w-auto flex-shrink-0 min-w-0">
              ...
              {hasPermission('sales.delete') && <button onClick={() => deleteData('enquiries', e.id)} className="flex-1 md:flex-none p-4 bg-slate-800 text-slate-500 rounded-2xl hover:bg-red-500/20 hover:text-red-500 transition-colors" title="Delete"><Trash2 size={20}/></button>}
            </div>
          </div>
        ))
      }
    ...
    <div className="space-y-6 text-left animate-in fade-in slide-in-from-bottom-4 duration-500 w-full max-w-7xl mx-auto">
      {showProductForm ? (
        <div className="bg-slate-900 p-6 md:p-12 rounded-[2.5rem] border border-slate-800 space-y-8">
          ...
          <div className="pt-8 border-t border-slate-800 text-left"><h4 className="text-white font-bold mb-4 flex items-center gap-2"><ImageIcon size={18} className="text-primary"/> Media Gallery</h4><FileUploader files={productData.media || []} onFilesChange={f => setProductData({...productData, media: f})} /></div>
        ...
        <>
          <div className="flex flex-col md:flex-row justify-between items-end gap-6 mb-8 text-left"><div className="space-y-2"><h2 className="text-3xl font-serif text-white">Catalog</h2><p className="text-slate-400 text-sm">Curate your collection of affiliate products.</p></div>{hasPermission('catalog.create') && <button onClick={() => { setProductData({}); setShowProductForm(true); setEditingId(null); }} className="px-8 py-4 bg-primary text-slate-900 rounded-xl font-black text-xs uppercase tracking-widest hover:bg-white transition-colors flex items-center gap-3 w-full md:w-auto justify-center"><Plus size={18} /> Add Product</button>}</div>
  ...
  const renderOrders = () => {
    const filteredOrders = orders.filter(o => {
        const matchesSearch = o.id.toLowerCase().includes(orderSearch.toLowerCase()) || o.customerName.toLowerCase().includes(orderSearch.toLowerCase()) || o.customerEmail.toLowerCase().includes(orderSearch.toLowerCase());
    ...
    return (
      <div className="space-y-6 text-left animate-in fade-in slide-in-from-bottom-4 duration-500 w-full max-w-7xl mx-auto">
         ...
         {viewingOrder && ( <div className="fixed inset-0 z-50 flex justify-end bg-black/60 backdrop-blur-sm animate-in fade-in duration-300" onClick={() => setViewingOrder(null)}><div className="w-full max-w-lg bg-slate-900 h-full shadow-2xl border-l border-slate-800 overflow-y-auto flex flex-col" onClick={e => e.stopPropagation()}><div className="p-6 border-b border-slate-800 flex justify-between items-start bg-slate-950/50"><div><h3 className="text-2xl font-serif text-white mb-1">Order Details</h3><div className="flex items-center gap-2 text-xs font-mono text-slate-400"><span>#{viewingOrder.id}</span><span>•</span><span>{new Date(viewingOrder.createdAt).toLocaleString()}</span></div></div><button onClick={() => setViewingOrder(null)} className="p-2 bg-slate-800 rounded-full text-slate-400 hover:text-white"><X size={20}/></button></div><div className="flex-grow p-6 space-y-8"><div className="space-y-4"><label className="text-[10px] font-black uppercase text-slate-500 tracking-widest">Order Status</label><select value={viewingOrder.status} onChange={(e) => handleOrderStatusUpdate(viewingOrder.id, e.target.value)} disabled={!hasPermission('sales.fulfillment')} className="w-full px-4 py-3 bg-slate-950 border border-slate-800 rounded-xl text-white outline-none focus:border-primary transition-all text-sm appearance-none cursor-pointer disabled:opacity-50">{['pending_payment', 'paid', 'processing', 'shipped', 'delivered', 'cancelled'].map(s => ( <option key={s} value={s}>{s.replace('_', ' ').toUpperCase()}</option> ))}</select></div><div className="space-y-4"><h4 className="text-white font-bold text-sm flex items-center gap-2 border-b border-slate-800 pb-2"><User size={16} className="text-primary"/> Customer</h4><div className="grid grid-cols-2 gap-4 text-xs text-slate-300"><div><span className="block text-slate-500 uppercase text-[9px] font-bold mb-1">Name</span>{viewingOrder.customerName}</div><div><span className="block text-slate-500 uppercase text-[9px] font-bold mb-1">Email</span>{viewingOrder.customerEmail}</div><div className="col-span-2"><span className="block text-slate-500 uppercase text-[9px] font-bold mb-1">Shipping Address</span><p className="leading-relaxed whitespace-pre-wrap">{viewingOrder.shippingAddress}</p></div></div></div><div className="space-y-4"><h4 className="text-white font-bold text-sm flex items-center gap-2 border-b border-slate-800 pb-2"><ShoppingBag size={16} className="text-primary"/> Items</h4><div className="space-y-3">{viewingOrder.items?.map((item: OrderItem) => ( <div key={item.id} className="flex justify-between items-center bg-slate-950/30 p-3 rounded-xl border border-slate-800/50"><div className="flex items-center gap-3"><div className="w-8 h-8 bg-slate-800 rounded-lg flex items-center justify-center text-xs font-bold text-white">{item.quantity}x</div><span className="text-sm text-slate-300">{item.productName}</span></div><span className="text-xs font-mono text-white">R {(item.price * item.quantity).toLocaleString()}</span></div> ))}<div className="flex justify-between items-center pt-2 text-sm font-bold text-white"><span>Total</span><span className="text-primary text-lg">R {viewingOrder.total.toLocaleString()}</span></div></div></div><div className="space-y-4 bg-slate-950 p-4 rounded-xl border border-slate-800"><h4 className="text-white font-bold text-sm flex items-center gap-2 mb-2"><Truck size={16} className="text-blue-500"/> Logistics</h4>{hasPermission('sales.fulfillment') ? (<div className="space-y-3"><div className="space-y-1"><label className="text-[9px] font-black uppercase text-slate-500 tracking-widest">Courier Name</label><input type="text" value={trackingInfo.courier} onChange={e => setTrackingInfo({...trackingInfo, courier: e.target.value})} className="w-full px-3 py-2 bg-slate-900 border border-slate-800 rounded-lg text-white text-xs outline-none focus:border-primary" placeholder="e.g. Aramex" /></div><div className="space-y-1"><label className="text-[9px] font-black uppercase text-slate-500 tracking-widest">Tracking Number</label><input type="text" value={trackingInfo.tracking} onChange={e => setTrackingInfo({...trackingInfo, tracking: e.target.value})} className="w-full px-3 py-2 bg-slate-900 border border-slate-800 rounded-lg text-white text-xs outline-none focus:border-primary" placeholder="e.g. DBP123456789" /></div><button onClick={handleSaveTracking} className="w-full py-3 bg-blue-600 text-white rounded-lg text-[10px] font-black uppercase tracking-widest hover:bg-blue-500 transition-colors">Update & Notify Client</button></div>) : (<div className="text-xs text-slate-500">Only Fulfillment staff can update logistics.</div>)}</div></div><div className="p-6 border-t border-slate-800 bg-slate-950/50 flex gap-4"><button onClick={() => printInvoice(viewingOrder)} className="flex-1 py-4 bg-slate-800 text-slate-300 rounded-xl font-bold uppercase text-xs tracking-widest hover:text-white hover:bg-slate-700 flex items-center justify-center gap-2"><Printer size={16}/> Print Invoice</button></div></div></div> )}
      </div>
    );
  };
  ...
  const renderSiteEditor = () => (
     <div className="space-y-6 w-full max-w-7xl mx-auto text-left">
       ...
       <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500 text-left">
          {[ 
            {id: 'brand', label: 'Identity', icon: Globe, desc: 'Logo, Colors, Slogan', perm: 'site.identity'}, 
            {id: 'nav', label: 'Navigation', icon: MapPin, desc: 'Menu Labels, Footer', perm: 'site.editor'}, 
            {id: 'home', label: 'Home Page', icon: Layout, desc: 'Hero, About, Trust Strip', perm: 'site.editor'}, 
            {id: 'collections', label: 'Collections', icon: ShoppingBag, desc: 'Shop Hero, Search Text', perm: 'site.editor'}, 
            {id: 'about', label: 'About Page', icon: User, desc: 'Story, Values, Gallery', perm: 'site.editor'}, 
            {id: 'contact', label: 'Contact Page', icon: Mail, desc: 'Info, Form, Socials', perm: 'site.editor'}, 
            {id: 'legal', label: 'Legal Text', icon: Shield, desc: 'Privacy, Terms, Disclosure', perm: 'site.legal'}, 
            {id: 'integrations', label: 'Integrations & Payments', icon: LinkIcon, desc: 'EmailJS, Tracking, Commerce', perm: 'site.integrations'} 
          ].map(s => {
           ...
           <div className="flex gap-4">
              <div className="px-4 py-2 bg-blue-500/10 border border-blue-500/20 rounded-xl flex items-center gap-3 text-left max-w-xs">
                 ...
                 <p className="text-[9px] text-blue-300 font-medium leading-tight">
                    <strong>Invitation Guide:</strong> Users must first be invited via the Supabase Auth Dashboard or sign up publicly.
                 </p>
              </div>
              {hasPermission('team.manage') && <button onClick={() => { setAdminData({ role: 'admin', permissions: [] }); setShowAdminForm(true); setEditingId(null); }} className="px-6 py-3 bg-primary text-slate-900 rounded-xl font-black text-xs uppercase tracking-widest w-full md:w-auto flex-shrink-0 h-fit self-center"><Plus size={16}/> New Member</button>}
           </div>
        </div>
        {showAdminForm ? (
               ...
               {activeEditorSection === 'integrations' && (
                  <>
                    ...
                    <div className="p-4 bg-slate-900 border border-slate-800 rounded-2xl mb-6">
                        <h4 className="text-white font-bold mb-4 flex items-center gap-2"><Mail size={16} /> Email Automation (EmailJS)</h4>
                        <div className="space-y-4">
                            <SettingField label="Service ID" value={tempSettings.emailJsServiceId || ''} onChange={v => updateTempSettings({ emailJsServiceId: v })} placeholder="service_..." />
                            <SettingField label="Template ID" value={tempSettings.emailJsTemplateId || ''} onChange={v => updateTempSettings({ emailJsTemplateId: v })} placeholder="template_..." />
                            <SettingField label="Public Key" value={tempSettings.emailJsPublicKey || ''} onChange={v => updateTempSettings({ emailJsPublicKey: v })} placeholder="user_..." />
                        </div>
                    </div>

                    <div className="p-4 bg-slate-900 border border-slate-800 rounded-2xl mb-6">
                        <h4 className="text-white font-bold mb-4 flex items-center gap-2"><ActivityIcon size={16} /> Analytics & Pixels</h4>
                        <div className="space-y-4">
                            <div className="grid md:grid-cols-2 gap-4">
                                <SettingField label="Google Analytics ID" value={tempSettings.googleAnalyticsId || ''} onChange={v => updateTempSettings({ googleAnalyticsId: v })} placeholder="G-..." />
                                <SettingField label="Facebook Pixel ID" value={tempSettings.facebookPixelId || ''} onChange={v => updateTempSettings({ facebookPixelId: v })} placeholder="123456..." />
                                <SettingField label="TikTok Pixel ID" value={tempSettings.tiktokPixelId || ''} onChange={v => updateTempSettings({ tiktokPixelId: v })} placeholder="C..." />
                                <SettingField label="Pinterest Tag ID" value={tempSettings.pinterestTagId || ''} onChange={v => updateTempSettings({ pinterestTagId: v })} placeholder="26..." />
                            </div>
                        </div>
                    </div>
