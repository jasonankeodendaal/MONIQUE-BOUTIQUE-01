...
const AnalyticsDashboard: React.FC<{ trafficEvents: TrafficLog[]; products: Product[]; stats: ProductStats[]; orders: Order[]; categories: Category[]; admins: AdminUser[]; user: any; isOwner: boolean }> = ({ trafficEvents, products, stats, orders, categories, admins, user, isOwner }) => {
  ...
  const generatePDFReport = () => {
    ...
    autoTable(doc, { startY: 40, head: [['Product Name', 'SKU', 'Views', 'Clicks', 'CTR', 'Price']], body: productRows, theme: 'grid', headStyles: { fillColor: secondaryColor, textColor: [255, 255, 255] as [number, number, number] }, styles: { fontSize: 9, cellPadding: 3 }, margin: { left: 25, right: 10 } });
...
const Admin: React.FC = () => {
  ...
  const renderEnquiries = () => (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 w-full max-w-7xl mx-auto text-left">
      ...
      {filteredEnquiries.length === 0 ? <div className="text-center py-20 bg-slate-900/50 rounded-[2.5rem] border border-dashed border-slate-800 text-slate-500">No enquiries found.</div> : filteredEnquiries.map(e => (<div key={e.id} className={`bg-slate-900 border transition-all rounded-[2rem] p-5 md:p-6 flex flex-col md:flex-row gap-6 text-left ${e.status === 'unread' ? 'border-primary/30' : 'border-slate-800'}`}><div className="flex-grow space-y-2 min-w-0"><div className="flex items-center gap-3"><h4 className="text-white font-bold truncate">{e.name}</h4><span className="text-[9px] font-black text-slate-500 uppercase flex-shrink-0">{new Date(e.createdAt).toLocaleDateString()}</span></div><p className="text-primary text-sm font-bold truncate">{e.email}</p><div className="p-4 bg-slate-800/50 rounded-2xl text-slate-400 text-sm italic">"{e.message}"</div></div><div className="flex gap-2 items-start">{hasPermission('sales.manage') && (<><button onClick={() => handleReplyToEnquiry(e)} className="p-4 bg-primary/20 text-primary rounded-2xl hover:brightness-110 transition-all"><Reply size={20}/></button><button onClick={() => toggleEnquiryStatus(e)} className={`p-4 rounded-2xl ${e.status === 'read' ? 'bg-slate-800 text-slate-500' : 'bg-green-500/20 text-green-500'}`}><CheckCircle size={20}/></button></>)}{hasPermission('sales.delete') && <button onClick={() => deleteData('enquiries', e.id)} className="p-4 bg-slate-800 text-slate-500 rounded-2xl hover:text-red-500"><Trash2 size={20}/></button>}</div></div>))}</div>
  );
  ...
  const ADMIN_TABS = [ 
    ...
    { id: 'subscribers', label: 'Audience', icon: Users, perm: 'sales.view' }, 
    ...
    { id: 'articles', label: 'Journal', icon: BookOpen, perm: 'content.categories' }, 
    ...
    { id: 'training', label: 'Academy', icon: GraduationCap, perm: 'training.view' }, 
