...
import { 
  ...
  Truck, Printer, Box, UserCheck, Repeat, Coins, Banknote, Power, TrendingDown, PieChart, CornerUpRight, ArrowDown, Youtube, Calculator, AlertCircle, RefreshCw, ShieldAlert, Binary, Unlock, Coins as CoinsIcon, ThumbsUp, ArrowUpDown, Table, FileDown, Presentation, Minus, Milestone
} from 'lucide-react';
import * as LucideIcons from 'lucide-react';
import jsPDF from 'jspdf';
import autoTable from 'jspdf-autotable';
import { GUIDE_STEPS, PERMISSION_TREE, TRAINING_MODULES } from '../constants';
import { Product, Category, CarouselSlide, MediaFile, SubCategory, SiteSettings, Enquiry, DiscountRule, SocialLink, AdminUser, PermissionNode, ProductStats, Order, OrderItem, SaveStatus, Review, Article, Subscriber, TrafficLog, TrainingModule, StoryMilestone } from '../types';
...
const SimpleLineChart = ({ data, color, height = 120 }: { data: number[], color: string, height?: number }) => {
  ...
  return (
    <div className="w-full relative overflow-visible group" style={{ height: `${height}px` }}>
        {hoveredIndex !== null && (
            <div className="absolute z-20 bg-slate-800 text-white text-[10px] font-bold px-2 py-1 rounded shadow-lg border border-slate-700 transform -translate-x-1/2 -translate-y-full pointer-none-none transition-all" style={{ left: `${points[hoveredIndex].x}%`, top: `${points[hoveredIndex].y}%`, marginTop: '-8px' }}>
...
const StoryMilestonesManager: React.FC<{ milestones: StoryMilestone[]; onChange: (milestones: StoryMilestone[]) => void }> = ({ milestones, onChange }) => {
  const handleUpdate = (id: string, field: keyof StoryMilestone, value: string) => {
    onChange(milestones.map(m => m.id === id ? { ...m, [field]: value } : m));
  };
  const handleAdd = () => {
    onChange([...milestones, { id: Date.now().toString(), year: '2025', title: 'New Achievement', description: 'Describe your journey milestone here...' }]);
  };
  return (
    <div className="space-y-6 w-full min-w-0">
      <div className="flex justify-between items-center mb-4">
        <label className="text-[10px] font-black uppercase text-slate-500 tracking-widest">Story Milestones</label>
        <button onClick={handleAdd} className="text-[10px] font-black uppercase text-primary flex items-center gap-1 hover:brightness-110">
          <Plus size={12}/> Add Milestone
        </button>
      </div>
      <div className="space-y-4">
        {milestones.map((milestone) => (
          <div key={milestone.id} className="bg-slate-900 p-6 rounded-2xl border border-slate-800 space-y-4 group">
            <div className="flex justify-between items-start">
               <div className="flex-grow grid md:grid-cols-2 gap-4">
                 <SettingField label="Year / Period" value={milestone.year} onChange={v => handleUpdate(milestone.id, 'year', v)} />
                 <SettingField label="Milestone Title" value={milestone.title} onChange={v => handleUpdate(milestone.id, 'title', v)} />
               </div>
               <button onClick={() => onChange(milestones.filter(m => m.id !== milestone.id))} className="p-2 text-slate-600 hover:text-red-500 ml-4">
                 <Trash2 size={18} />
               </button>
            </div>
            <SettingField label="Description" value={milestone.description} onChange={v => handleUpdate(milestone.id, 'description', v)} type="textarea" rows={3} />
          </div>
        ))}
        {milestones.length === 0 && (
          <div className="text-center py-10 border-2 border-dashed border-slate-800 rounded-3xl text-slate-600">
             <Milestone size={32} className="mx-auto mb-2 opacity-20" />
             <p className="text-xs font-bold uppercase tracking-widest">No milestones recorded</p>
          </div>
        )}
      </div>
    </div>
