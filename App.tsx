    ...
    <>
      <footer className="bg-slate-900 text-slate-400 py-8 md:py-24 border-t border-slate-800 relative z-20">
        <div className="max-w-7xl mx-auto px-4 md:px-12">
          <div className="grid grid-cols-2 md:grid-cols-5 gap-8 md:gap-12 mb-8 md:mb-16 text-left items-start">
            <div className="col-span-2 md:col-span-2 space-y-4 md:space-y-8">
            ...
            <div className="space-y-4 md:space-y-8 col-span-2 md:col-span-1">
              ...
              >
                <input 
                  ...
                  required
                  className="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-3 text-[10px] md:text-sm text-white focus:outline-none focus:border-primary transition-colors"
                />
                <button type="submit" className="w-full bg-primary text-slate-900 font-bold text-[8px] md:text-xs uppercase tracking-widest py-3 rounded-xl hover:bg-white transition-colors">
                  Subscribe
                </button>
              </form>
            </div>
...
const App: React.FC = () => {
  ...
  const calculateStorage = useCallback(() => {
      ...
      const totalRecs = products.length + categories.length + subCategories.length + heroSlides.length + enquiries.length + admins.length + trainingModules.length;
  ...
  }, [settings, products, categories, subCategories, heroSlides, enquiries, admins, stats, trainingModules]);
  ...
  const addSystemLog = async (type: SystemLog['type'], target: string, message: string, sizeBytes?: number, status: 'success' | 'failed' = 'success') => {
    ...
    if (isSupabaseConfigured) {
      try { await supabase.from('system_logs').insert([newLog]); } catch (e) {}
    }
  };
