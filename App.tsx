...
const Footer: React.FC = () => {
  ...
  return (
    <>
      <footer className="bg-slate-900 text-slate-400 py-10 md:py-14 border-t border-slate-800 relative z-20">
        <div className="max-w-7xl mx-auto px-6 md:px-12">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-12 md:gap-20 mb-10 text-left items-start">
            <div className="md:col-span-2 space-y-6">
              <div className="flex items-center space-x-6">
                 {settings.companyLogoUrl ? (
                  <img src={settings.companyLogoUrl} alt={settings.companyName} className="h-12 md:h-16 w-auto object-contain drop-shadow-lg" />
                ) : (
                  <div className="w-12 h-12 rounded-2xl flex items-center justify-center text-white font-black bg-primary text-xl shadow-xl">
                    {settings.companyLogo}
                  </div>
                )}
                <div className="flex flex-col -space-y-1">
                  <span className="text-white text-xl md:text-2xl font-serif font-bold tracking-tighter">{settings.companyName}</span>
                  <span className="text-[9px] md:text-[10px] font-black uppercase tracking-[0.3em] text-primary">{settings.slogan}</span>
                </div>
              </div>
                  ...
                  {(settings.socialLinks || []).map((link) => (
                    ...
                    >
                      ...
                      {link.iconUrl ? (
                        <img 
                          src={link.iconUrl} 
                          className="w-6 h-6 md:w-8 md:h-8 object-contain transition-all duration-500 group-hover:scale-125 opacity-100 group-hover:brightness-110 drop-shadow-[0_0_12px_rgba(255,255,255,0.1)]" 
                          alt={link.name}
                        />
                      ) : (
            ...
            <div className="space-y-6">
              <h4 className="text-white font-bold text-sm uppercase tracking-widest border-b border-slate-800 pb-3">{settings.footerNavHeader || 'Explore'}</h4>
            ...
            <div className="space-y-6">
              <h4 className="text-white font-bold text-sm uppercase tracking-widest border-b border-slate-800 pb-3">{settings.footerPolicyHeader || 'Institutional'}</h4>
              <ul className="space-y-3 text-sm font-light">
          ...
          <div className="pt-8 border-t border-slate-800 text-[10px] uppercase tracking-[0.2em] font-medium text-slate-500 flex flex-col md:flex-row justify-between items-center gap-6">
            ...
            <div className="flex flex-wrap justify-center items-center gap-6 md:gap-8 order-1 md:order-2">
               <button 
                  ...
                  className="flex items-center gap-3 px-4 py-2 bg-white/5 hover:bg-white/10 rounded-full border border-white/10 transition-all group shadow-2xl active:scale-95"
               >
                  <span className="text-[10px] font-bold text-slate-400 group-hover:text-white transition-colors">System By</span>
                  <img src="https://i.ibb.co/ZR8bZRSp/JSTYP-me-Logo.png" alt="JSTYP.me" className="h-4 w-auto opacity-80 group-hover:opacity-100 transition-all brightness-0 invert" />
               </button>

               <div className="flex items-center gap-4">
                  <div className="flex items-center gap-2">
                    <div className={`w-1.5 h-1.5 rounded-full ${connectionHealth?.status === 'online' ? 'bg-green-500 animate-pulse' : 'bg-red-500 shadow-[0_0_8px_#ef4444]'}`}></div>
                    <span className="text-[9px] font-mono text-slate-400">{connectionHealth?.latency || 0}ms</span>
                  </div>
                  ...
                  <div className="flex items-center gap-4 border-l border-slate-800 pl-4">
                    ...
                    />
                    <Link to={user ? "/admin" : "/login"} className="opacity-40 hover:opacity-100 hover:text-white transition-all text-[10px] font-black tracking-widest">
                      PORTAL
                    </Link>
                  </div>
               </div>
...
const App: React.FC = () => {
  ...
  const calculateStorage = useCallback(() => {
      // Fix: replaced undefined variable 'someCategoryData' with 'subCategories'
